...
def get_random_sentence(tx):
    statement = "MATCH(s:Sentence) -[:sentence_in]-> (w:WrittenWork) -[:written_by]-> (a:Author) RETURN w.name, s.text, rand() as r \
                    ORDER BY r limit 10"
...
def get_random_sentence_by_author(tx, author):
    statement = "MATCH(s:Sentence) -[:sentence_in]-> (w:WrittenWork) -[:written_by]-> (a:Author)  \
                   where a.name = '" + author + "' RETURN w.name, s.text, rand() as r ORDER BY r limit 10"
    result = tx.run(statement)
    return result
...
def get_random_sentence_by_work(tx, work):
    statement = "MATCH(s:Sentence) -[:sentence_in]-> (w:WrittenWork) -[:written_by]-> (a:Author)  \
                where w.name = '" + work + "' RETURN w.name, s.text, rand() as r ORDER BY r limit 10"
    result = tx.run(statement)
    return result
